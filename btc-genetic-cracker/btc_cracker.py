#!/usr/bin/env python3
"""
Script: btc_cracker.py
Author: CanalQb
License: MIT

Cracker genético de chaves Bitcoin com algoritmo evolucionário (DEAP).
Busca por chaves privadas correspondentes a endereços Bitcoin alvo
usando um algoritmo genético que evolui uma população de candidatos.
...
LOG_PATH = SCRIPT_DIR / "btc_cracker.log"
logging.basicConfig(
...
# --- Configurações ---
OUTPUT_FILE = SCRIPT_DIR / "carteiras.txt"

# Lista de endereços alvo (puzzles Bitcoin conhecidos)
DATA_ADDRESS = [
...
def save_wif_and_btc(text, file=None):
    """
    Extrai WIFs e endereços BTC de um texto e salva em arquivo.

    Args:
        text: Texto contendo WIFs e endereços
        file: Caminho do arquivo (default: carteiras.txt)
    """
    file = file or OUTPUT_FILE
    regex_wif = r'\b[5KL][1-9A-HJ-NP-Za-km-z]{50,51}\b'
    regex_btc = r'\b(?:1|3|bc1)[a-zA-HJ-NP-Z0-9]{25,39}\b'

    wifs = re.findall(regex_wif, text)
    btc_addresses = re.findall(regex_btc, text)
...
def get_config_by_length_improved(length):
    """Retorna configuração de parâmetros do AG com base no tamanho do cromossomo."""
    configs = [
...
def _get_key_from_individual(individual: List[int]) -> Tuple[Optional[Key], str]:
    """Cria uma Key a partir de um indivíduo (lista de bits)."""
    ...
    try:
        ...
        logger.debug(f"Erro ao criar Key: {e}")
        return None, binary_secret


def evaluate_individual(individual: List[int]) -> Tuple[int]:
    """Avalia um indivíduo - retorna 1 se o endereço está na lista alvo."""
...
def get_individual_details(individual: List[int]) -> Tuple[int, str, str, str]:
    """Extrai detalhes de um indivíduo bem-sucedido."""
...
def apply_crossover_and_mutation(offspring, toolbox, p_crossover, p_mutation):
    """Aplica crossover e mutação a uma população de descendentes."""
...
def run_genetic_algorithm(target_length: int, pop_size: int = 400,
                          max_gen: int = 400, seed: int = 9):
    """
    Executa o algoritmo genético para buscar chaves Bitcoin.

    Args:
        target_length: Tamanho do cromossomo em bits
        pop_size: Tamanho da população
        max_gen: Número máximo de gerações
        seed: Semente aleatória
    """
    config = get_config_by_length_improved(target_length)
    # Usa config se não sobrescrito explicitamente
    if pop_size == 400:
    ...
    logger.info(f"AG iniciado: bits={target_length}, pop={pop_size}, "
                f"max_gen={max_gen}, p_cx={P_CX}, p_mut={P_MUT}")
    ...
    toolbox.register("individualCreator", tools.initRepeat, creator.Individual,
                     toolbox.zeroOrOne, target_length)
    ...
    while not found:
        logger.info(f"Iniciando AG: {target_length} bits | Crossover: {p_crossover_current:.4f}")
            ...
            while best.fitness.values[0] < 1 and generation < max_gen:
                ...
                if generation % 10 == 0 or best.fitness.values[0] == 1:
                    ...
                    if best.fitness.values[0] == 1:
                        ...
                        msg = f"[✔] Gen {generation}: WIF={wif} End={addr} Tempo={elapsed:.2f}s"
                        logging.info(msg)
                        print(msg)
                        save_wif_and_btc(f"WIF: {wif} {addr}")
                        found = True
                    else:
                        print(f"[ ] Gen {generation}: Nenhum endereço. Tempo: {elapsed:.2f}s", end='\r')

        if not found:
            p_crossover_current = min(p_crossover_current + 0.001, 1.0)
            logger.info(f"Nenhum endereço encontrado. Novo P_CROSSOVER: {p_crossover_current:.4f}")


def solicitar_target_length() -> int:
    """Solicita o tamanho do cromossomo ao usuário."""
    while True:
        try:
            valor = int(input("Informe o TARGET_SEQUENCE_LENGTH (1 a 256): "))
            if 1 <= valor <= 256:
                print(f"TARGET_SEQUENCE_LENGTH definido como {valor}")
                return valor
            else:
                print("Insira um número entre 1 e 256.")
        except ValueError:
...
def main():
    parser = argparse.ArgumentParser(
        description='Genetic algorithm cracker for Bitcoin puzzle addresses',
...
"""
    ...
    parser.add_argument('--bits', '-b', type=int, default=8,
                        help='Target chromosome length in bits (1-256)')
    parser.add_argument('--pop-size', type=int, default=400,
                        help='Population size (default: auto from config)')
    parser.add_argument('--max-gen', type=int, default=400,
                        help='Max generations (default: auto from config)')
    parser.add_argument('--seed', type=int, default=9, help='Random seed')

    args = parser.parse_args()

    if args.bits < 1 or args.bits > 256:
        print("[!] bits deve estar entre 1 e 256")
        sys.exit(1)

    logger.info("Iniciando btc_cracker.py")
    print(f"[*] Iniciando algoritmo genético para {args.bits} bits")

    if args.pop_size != 400:
        run_genetic_algorithm(args.bits, pop_size=args.pop_size,
                              max_gen=args.max_gen, seed=args.seed)
    else:
        run_genetic_algorithm(args.bits, max_gen=args.max_gen, seed=args.seed)
