#!/usr/bin/env python3
"""
Script: btc_bruteforce_ga.py
Author: CanalQb
License: MIT

Brute force com algoritmo genético para busca de chaves Bitcoin.
Combina busca genética com verificação de endereços e suporte a
ice_secp256k1.dll para aceleração.

Usage:
    python btc_bruteforce_ga.py
    python btc_bruteforce_ga.py --bits 64 --pop-size 500 --max-gen 1000
"""
...
LOG_PATH = SCRIPT_DIR / "btc_bruteforce_ga.log"
logging.basicConfig(
...
def get_key_from_individual(individual: List[int]) -> Tuple[Optional[Key], str]:
...
def fitness_function(individual: List[int]) -> Tuple[int]:
    """Função de fitness: verifica se o endereço está na lista alvo."""
    key, _ = get_key_from_individual(individual)
    if key and key.address in DATA_ADDRESS:
        return (1,)
    return (0,)


def get_individual_details(individual: List[int], target_address: str) -> Tuple[int, str, str, str]:
    """Extrai detalhes completos de um indivíduo."""
    key, binary_secret = get_key_from_individual(individual)
    if not key:
        return 0, "", binary_secret, ""
    guess = key.address
    score = sum(1 for a, b in zip(target_address, guess) if a == b)
    return score, guess, binary_secret, key.to_wif()
...
def run_genetic_algorithm(pop_size: int = POPULATION_SIZE,
    ...
    Args:
        ...
        target_len: Comprimento da sequência alvo
    """
    target_address = random.choice(DATA_ADDRESS)
    print(f"🎯 Endereço alvo: {target_address}")
    logger.info(f"Iniciando GA: target={target_address}, pop={pop_size}, max_gen={max_gen}")

    # Atualiza tamanho do indivíduo
    ...
    with Pool() as pool:
        ...
        print(f"🏁 Fitness inicial: {max_fitness}")

        start_time = time.time()

        while max_fitness < 1 and generation < max_gen:
            generation += 1
            ...
            if generation % 10 == 0 or max_fitness == 1:
                score, addr, secret, wif = get_individual_details(best_individual, target_address)
                elapsed = time.time() - start_time
                print(f"Gen {generation}: Fitness={max_fitness} Score={score} "
                      f"WIF={wif[:20]}... Time={elapsed:.2f}s")
    ...
    print(f"Melhor score: {score}")
    print(f"Melhor endereço: {addr}")
    print(f"Segredo (bin): {secret}")
    print(f"Segredo (int): {int(secret, 2)}")
    print(f"WIF: {wif}")

    logger.info(f"GA concluído: gen={generation}, score={score}")


def main():
    parser = argparse.ArgumentParser(
        description='Brute force genético para busca de chaves Bitcoin',
...
"""
    ...
    parser.add_argument('--bits', '-b', type=int, default=TARGET_LEN,
                        help=f'Comprimento do cromossomo (default: {TARGET_LEN})')
    ...
    parser.add_argument('--max-gen', type=int, default=MAX_GENERATIONS,
                        help=f'Máximo de gerações (default: {MAX_GENERATIONS})')
    parser.add_argument('--seed', type=int, default=RANDOM_SEED,
                        help=f'Semente aleatória (default: {RANDOM_SEED})')

    args = parser.parse_args()

    print(f"🚀 BRUTE FORCE GENÉTICO - Bitcoin")
    print(f"ICE DLL: {'✅' if ICE_LIB else '❌'}")
    print(f"Bits: {args.bits}, Pop: {args.pop_size}, Gens: {args.max_gen}")
    print("=" * 50)

    random.seed(args.seed)
    run_genetic_algorithm(pop_size=args.pop_size, max_gen=args.max_gen,
                          target_len=args.bits)
